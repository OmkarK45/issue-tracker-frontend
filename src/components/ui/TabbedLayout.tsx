...
export function TabbedLayout({ navigation }: TabbedLayoutProps) {
	...
	function handleChange(idx: number) {
		const path = navigation[idx].href
		setCurrentPath(path)
	...
	return (
		<Tab.Group
			defaultIndex={navigation.findIndex((x) => x.href === currentPath)}
			onChange={(idx) => handleChange(idx)}
			vertical
		>
			<aside className="py-6 w-64 px-2 sm:px-6 lg:py-0 lg:px-0 lg:col-span-3">
				<nav
					aria-label="Sidebar"
